# ===============================================
# 💬 Banking Intention Router (Phase 2 — Final Unified + Dual Voice)
# -----------------------------------------------
# ✅ Unified with Phase 1 (Whisper model)
# ✅ DIAG logging (env default, per-request override)
# ✅ Dual Voice: REPLY_MODE (env default, per-request override)
#    - 0 → Static-first (studio WAVs in static/tts), fallback to TTS
#    - 1 → Dynamic Edge-TTS (always synth)
# ✅ Always reads balance_amount.txt if present
# ✅ Always returns static_voice_file for instant playback
# ✅ Archives interaction metadata
# ===============================================
...
def classify_intent(
    ...
    dlog(f"🔧 DIAG={int(req_diag)}  REPLY_MODE={'1(dynamic)' if reply_mode_dynamic else '0(static)'}")

    result = classify_text(text)
    domain_action = get_domain_action(result["intent"])
    result.update(domain_action)

    # ✅ Always include static studio WAV reference
